...
import { convertToUint8Array, convertToArrayBuffer, sha1BytesSync, nextRandomInt, bytesCmp, bytesToHex, bytesFromArrayBuffer, bytesToArrayBuffer, longToBytes, uintToInt, rshift32 } from '../../bin';
...
var xhrSendBuffer = !isNode && !('ArrayBufferView' in window);

var storeIntString = writer => (value, field) => {
  switch (typeof value) {
    case 'string':
      return WriteMediator.bytes(writer, value, `${field}:string`);
    case 'number':
      return WriteMediator.int(writer, value, field);
    default:
...
export class NetworkerThread {
  ...
  wrapMtpCall(method, params, options) {
    ...
    var message = new NetMessage(seqNo, serializer.getBytes(true));
  ...
  parseResponse(responseBuffer) {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (!bytesCmp(authKeyID, _this2.authKeyID)) {
      ...
      var dataWithPadding = yield CryptoWorker.aesDecrypt(encryptedData, keyIv[0], keyIv[1]);
      ...
      var messageID = ReadMediator.long(deserializer.typeBuffer, 'message_id');

      var isInvalidSession = !bytesCmp(sessionID, _this2.sessionID) && (!_this2.prevSessionID ||
      //eslint-disable-next-line
      !bytesCmp(sessionID, _this2.prevSessionID));
      if (isInvalidSession) {
  ...
  cleanupSent() {
    ...
    for (var _iterator3 = this.state.sentIterator(), _isArray3 = Array.isArray(_iterator3), _i3 = 0, _iterator3 = _isArray3 ? _iterator3 : _iterator3[Symbol.iterator]();;) {
      ...
      if (message.notContentRelated && !this.state.hasPending(msgID))
        ...
        this.state.deleteSent(message);else if (message instanceof NetContainer) {
      ...
      case 'bad_msg_notification':
        {
          ...
          if (message.error_code == 16 || message.error_code == 17) {
            if (applyServerTime(rshift32(messageID))) {
              log(`Update session`)();
              this.updateSession();
            }
...
var _initialiseProps = function () {
  ...
  this.checkLongPollCond = () => this.longPollPending && this.longPollPending > tsNow() || !!this.offline || akStopped;
  ...
  this.sendLongPoll = _asyncToGenerator(function* () {
    var maxWait = 25000;
    _this3.longPollPending = tsNow() + maxWait;
  ...
  function* _ref11(event) {
    ...
    smartTimeout.cancel(_this3.checkConnectionPromise);
    ...
    var pingMessage = new NetMessage(_this3.generateSeqNo(true), serializer.getBytes());
    ...
    _this3.checkConnectionPromise = smartTimeout(_this3.checkConnection, delay);
  ...
  this.performSheduledRequest = _asyncToGenerator(function* () {
    ...
    if (hasApiCall && !hasHttpWait) {
      ...
      messages.push(new NetMessage(_this3.generateSeqNo(), serializer.getBytes()));
    }
    ...
    if (messages.length > 1) {
      ...
      message = new NetContainer(_this3.generateSeqNo(true), container.getBytes(true), innerMessages);
  ...
  function* _ref20(message, options = {}) {
    ...
    WriteMediator.intBytes(dataBox, _this3.serverSalt, 64, 'salt');
    WriteMediator.intBytes(dataBox, _this3.sessionID, 64, 'session_id');
    ...
    var encryptedBytes = yield CryptoWorker.aesEncrypt(bytes, keyIv[0], keyIv[1]);

    var request = _this3.Serialization({ startMaxLength: encryptedBytes.byteLength + 256 });
    var requestBox = request.writer;
    WriteMediator.intBytes(requestBox, _this3.authKeyID, 64, 'auth_key_id');
    ...
    try {
      var result = yield httpClient.post(url, requestData, options);
      return !result.data || !result.data.byteLength ? Promise.reject(new ErrorBadResponse(url, result)) : result;
    } catch (error) {
...
export var NetworkerFabric = (appConfig, { Serialization, Deserialization }, storage, emit) => chooseServer => (dc, authKey, serverSalt, options = {}) => new NetworkerThread({
  appConfig,
  chooseServer,
  Serialization,
  Deserialization,
