...
import { uintToInt, intToUint, bytesToHex, gzipUncompress, bytesToArrayBuffer, longToInts, lshift32, stringToChars } from '../bin';

import { WriteMediator, ReadMediator } from './mediator';
import Layout, { getFlags, isSimpleType, getTypeProps } from '../layout';
import { TypeBuffer, TypeWriter, getNakedType, getString, getTypeConstruct } from './type-buffer';
...
export class Serialization {
  constructor({ mtproto, startMaxLength }, api, mtApi) {
    ...
    if (!apiLayer) apiLayer = new Layout(api);
    if (!mtLayer) mtLayer = new Layout(mtApi);
  }
  ...
  storeMethod(methodName, params) {
    ...
    WriteMediator.int(this.writer, intToUint(`${pred.id}`), `${methodName}[id]`);
    if (pred.hasFlags) {
  ...
  storeObject(obj, type, field) {
    ...
    if (Array.isArray(obj)) {
      if (type.substr(0, 6) == 'Vector') WriteMediator.int(this.writer, 0x1cb5c415, `${field}[id]`);else if (type.substr(0, 6) != 'vector') {
    ...
    if (!is(Object, obj)) throw new Error(`Invalid object for type ${type}`);
    ...
    if (!isBare) WriteMediator.int(this.writer, intToUint(constructorData.id), `${field}.${predicate}[id]`);
...
export class Deserialization {
  ...
  fetchPacked(type, field = '') {
    var compressed = ReadMediator.bytes(this.typeBuffer, `${field}[packed_string]`);
